...
本地 **PreToolUse hook 后端** 服务于 Codex: 拦截 Codex 的工具调用, 通过「正则 / 路径规则先判 → Codex Haiku 4.5 兜底」决定 approve / block / ask。
...
Codex
  ...
  → FastAPI (uvicorn) [src/classifier/main.py]
       ...
       └─ Codex Haiku 4.5 fallback: 规则无命中时用 tool_use 结构化分类
...
### 双协议 Decision 字段
`Decision` schema 同时携带 `decision` (legacy: `approve`/`block`) 与 `hookSpecificOutput.permissionDecision` (new: `allow`/`deny`/`ask`), 任意版本的 Codex 都能解析。**新增 endpoint 必须保持这两个字段都填**。

### Fail-open 默认
`fail_open_on_error=true` 是默认行为。任何分类器自身出错都应当 allow 用户, 不要 deny。通过 YAML/`.env` 切换 fail-closed, 不要改代码。
...
### 环境变量与配置层叠
优先级 (后覆盖前): DEFAULTS → `config/default.yaml` → `~/.config/clf/rules.yaml` (或 `$CLF_RULES_FILE`) → `CLF_<SECTION>_<KEY>` 环境变量。`ANTHROPIC_API_KEY` 永远从环境变量读, 不进 YAML。
...
# 装 hook 到 ~/.Codex/settings.json
bash scripts/install_hook.sh
powershell -File scripts/install_hook.ps1
...
- `tail -f logs/decisions.jsonl | jq` → 全量审计 (tool / decision / matched_rule / took_ms / request_id)。
- `CLF_LOG_LEVEL=DEBUG bash scripts/dev.sh` → 详细日志 (包含 Codex fallback 调试)。
- 临时绕过 hook: 直接编辑 `~/.Codex/settings.json` 删掉 `PreToolUse` 那一项, 或用环境变量 `CLF_BRIDGE_TOKEN` 强制鉴权失败 (会触发 401 → fail-open)。
